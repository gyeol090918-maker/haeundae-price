  ...
  form.addEventListener('submit',event=>{
    ...
    if(window.kakao?.maps?.services){
      ...
      places.keywordSearch(`해운대 ${query}`,(data,status)=>{
        ...
        list.innerHTML=actual.map(place=>{const name=place.place_name.replace(/</g,'&lt;');const category=(place.category_name||'음식점').split(' > ').pop();const address=place.road_address_name||place.address_name||'해운대 인근';const known=realHaeundaeStores.find(store=>compact(store[0]).includes(compact(place.place_name))||compact(place.place_name).includes(compact(store[0])));const comparison=known?(()=>{const [state,tag]=storeStatus(known[1],known[2]);return {label:`${known[1]} · ${state}`,tag,detail:`대표 메뉴 ${won(known[2])} · 해운대 중앙값과 비교해 ${state}`};})():{label:`${category} · 가격 정보 없음`,tag:'tag-적정',detail:'대표 메뉴 가격이 등록되면 해운대 중앙값과 비교할 수 있습니다.'};return `<article class="place-card search-result-card"><div class="place-top"><h3>${name}</h3><span class="price-tag ${comparison.tag}">${comparison.label}</span></div><p>${category} · ${address}</p><p class="price-judgement">${comparison.detail}</p><a class="place-map-link" target="_blank" rel="noopener" href="${place.place_url}">지도에서 보기 →</a></article>`;}).join('');
