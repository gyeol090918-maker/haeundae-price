...
const storePriceAliases={'금수복국 해운대본점':['금수복국','금수복국해운대','금수복국본점'],'해운대기와집대구탕':['기와집대구탕','해운대기와집','기와집'],'속씨원한대구탕 미포본점':['속씨원한대구탕','속씨원한대구탕미포'],'해운대암소갈비집':['해운대암소갈비','암소갈비집','암소갈비'],'오복돼지국밥':['오복국밥','오복돼지국밥해운대','해운대오복돼지국밥'],'형제전통돼지국밥':['형제돼지국밥'],'해운대 가야밀면':['가야밀면','해운대가야밀면'],'춘하추동밀면 해운대점':['춘하추동밀면','춘하추동밀면해운대'],'미포집 해운대본점':['미포집','미포집해운대'],'청사포 조개구이':['청사포조개구이','청사포조개'],'해목 해운대점':['해목','해목해운대'],'해성막창집 본점':['해성막창','해성막창집'],'해운대 개미집':['개미집','해운대개미집'],'나가하마만게츠':['나가하마만게츠해운대','만게츠'],'해운대 상국이네':['상국이네','해운대상국이네'],'해운대 버거샵':['버거샵','해운대버거샵'],'해운대 원조전복죽':['원조전복죽','해운대전복죽']};
function findRegisteredPriceStore(placeName){const normalize=value=>String(value||'').toLowerCase().replace(/[\s()·,.\-_/]/g,'');const name=normalize(placeName);if(name.length<2)return null;return realHaeundaeStores.find(store=>[store[0],...(storePriceAliases[store[0]]||[])].some(candidate=>{const key=normalize(candidate);return key.includes(name)||name.includes(key);}))||null;}
realHaeundaeStores.push(
  ['극동돼지국밥','돼지국밥',11000,4.4],['석대국밥','돼지국밥',10000,4.3],['진돼지국밥','돼지국밥',9000,4.2],['수변최고돼지국밥 해운대점','돼지국밥',11000,4.4],['엄용백돼지국밥','돼지국밥',13000,4.5],['합천돼지국밥','돼지국밥',10000,4.3],['국봉수백진국','돼지국밥',11000,4.4],['해운회관','모둠회 2인',68000,4.4],['부산횟집 해운대본점','모둠회 2인',70000,4.3]
);
Object.assign(storePriceAliases,{'극동돼지국밥':['극동국밥'],'석대국밥':['석대가마솥국밥'],'진돼지국밥':['진국밥'],'수변최고돼지국밥 해운대점':['수변최고돼지국밥','수변최고국밥'],'엄용백돼지국밥':['엄용백','엄용백국밥'],'합천돼지국밥':['합천국밥'],'국봉수백진국':['국봉수백','국봉국밥'],'해운회관':['해운회관해운대'],'부산횟집 해운대본점':['부산횟집','부산횟집해운대']});
...
// 홈 지도 검색 결과에서는 지도 이름이 지점명과 달라도 등록된 가격을 우선 표시한다.
function installRegisteredPriceSearch(){
  const oldForm=document.querySelector('#nearbySearch'); if(!oldForm)return;
  const form=oldForm.cloneNode(true); oldForm.replaceWith(form);
  const input=form.querySelector('#homeRestaurantFinder'); if(!input)return;
  const compact=value=>String(value||'').replace(/\s/g,'').toLowerCase();
  const render=(place, fallbackMenu='음식점')=>{const name=String(place.place_name||place.name||'').replace(/</g,'&lt;');const known=findRegisteredPriceStore(place.place_name||place.name);const category=(place.category_name||fallbackMenu).split(' > ').pop();const address=place.road_address_name||place.address_name||'해운대 인근';const price=known?(()=>{const [state,tag]=storeStatus(known[1],known[2]);return `<span class="price-tag ${tag}">대표 메뉴 ${known[1]} ${won(known[2])} · ${state}</span><p class="price-judgement">해운대 ${known[1]} 중앙값과 비교한 앱 등록 가격입니다.</p>`;})():`<span class="price-tag tag-정보없음">앱 가격 데이터 미등록</span><p class="price-judgement">앱에 검증된 메뉴 가격이 아직 등록되지 않았습니다.</p>`;return `<article class="place-card search-result-card"><div class="place-top"><h3>${name}</h3>${price}</div><p>${category} · ${address}</p><a class="place-map-link" target="_blank" rel="noopener" href="${place.place_url||`https://map.naver.com/p/search/${encodeURIComponent('해운대 '+name)}`}">지도에서 보기 →</a></article>`;};
  form.addEventListener('submit',event=>{event.preventDefault();const query=input.value.trim();if(!query)return;const list=document.querySelector('#placeList'),title=document.querySelector('#nearbyTitle'),sub=document.querySelector('#nearbySubtitle');document.querySelector('#actualMap').src=`https://www.google.com/maps?q=${encodeURIComponent('해운대 '+query)}&output=embed`;title.textContent=`“${query}” 실제 인근 식당`;sub.textContent='등록 가격을 확인하는 중입니다…';const local=realHaeundaeStores.filter(store=>compact(store[0]).includes(compact(query))||compact(store[1]).includes(compact(query))).map(store=>({place_name:store[0],category_name:store[1],address_name:'해운대 인근'}));if(!window.kakao?.maps?.services){list.innerHTML=local.map(place=>render(place)).join('')||'<p class="source-note">실제 장소 정보를 불러오지 못했습니다.</p>';sub.textContent=`${local.length}개 등록 가격 식당`;return;}const places=new kakao.maps.services.Places();places.keywordSearch(`해운대 ${query}`,(data,status)=>{const results=status===kakao.maps.services.Status.OK?data:local;const combined=[...results,...local.filter(x=>!results.some(y=>compact(y.place_name)===compact(x.place_name)))];list.innerHTML=combined.slice(0,15).map(place=>render(place,query)).join('')||'<p class="source-note">검색 결과가 없습니다.</p>';const priced=combined.filter(place=>findRegisteredPriceStore(place.place_name)).length;sub.textContent=`${combined.length}개 식당 · 가격 정보 ${priced}곳 표시`;},{x:129.1604,y:35.1587,radius:5000,size:15,sort:kakao.maps.services.SortBy.DISTANCE});});
}
...
// 첫 화면은 등록 식당만 즉시 표시한다. 대량의 추가 매장 요청은 새로고침 속도를 늦추므로 보류한다.
const homepageFetch=window.fetch.bind(window);
window.fetch=(resource,...options)=>{const url=typeof resource==='string'?resource:resource?.url||'';if(url.includes('overpass-api.de/api/interpreter')&&url.includes('timeout%3A25'))return Promise.resolve({ok:true,json:()=>Promise.resolve({elements:[]})});return homepageFetch(resource,...options);};
